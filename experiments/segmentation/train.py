...
from encoding.utils import save_checkpoint
from encoding.nn import SegmentationLosses, SyncBatchNorm
from encoding.parallel import DataParallelModel, DataParallelCriterion
from encoding.datasets import get_dataset
from encoding.models import get_segmentation_model
from cs_data_loader import *
from encoding.utils import LR_Scheduler_Head
...
class Trainer():
    def __init__(self, args):
        ...
        train_set = CSDataset('../../../semantic-foreground-inpainting/train2.csv', transform=transforms.Compose([Rescale(img_size), CSToTensor()]))
        val_set = CSDataset('../../../semantic-foreground-inpainting/val2.csv', transform=transforms.Compose([Rescale(img_size), CSToTensor()]))
        self.trainloader = DataLoader(train_set, batch_size=8, shuffle=True, num_workers=6, drop_last=True)
        self.valloader = DataLoader(val_set, batch_size=1, shuffle=False, num_workers=6, drop_last=False)
    ...
    def training(self, epoch):
        ...
        for i, temp_batch in enumerate(tbar):
            ...
            target=(temp_foregd,temp_partial_bkgd)
            self.scheduler(self.optimizer, i, epoch, self.best_pred)
            self.optimizer.zero_grad()
            #print("build")
            outputs = self.model(temp_rgb)
            #print("loss")
            #print(len(outputs))
            #print(temp_partial_bkgd.detach().cpu().numpy().shape)
            #print(np.max(outputs1.detach().cpu().numpy()))
            #print(np.max(temp_partial_bkgd.detach().cpu().numpy()))
            loss = self.criterion(outputs, temp_partial_bkgd, temp_foregd)
